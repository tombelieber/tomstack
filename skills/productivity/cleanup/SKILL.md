...
1. `orphan-reaper` when available on `PATH`.
2. `bash ~/.claude/plugins/marketplaces/tomstack/orphan-reaper/cli.sh` when that legacy plugin path exists.
3. `npx --yes orphan-reaper` when no installed command is available.

On first invocation, probe with `version --json`, then store the working command
